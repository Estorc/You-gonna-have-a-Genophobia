...
(() => {
    ...
    function createTextPictureBitmap(text) {
        ...
        tempWindow.move(-8, -8, size.width+16, size.height+16);
        tempWindow.createContents();
		
		
		if (text.contains('\FontChange<kk-lyricsfont>')) {
		tempWindow.changeTextColor("#290000")
		tempWindow.changeOutlineColor("#290000")
		
		for (let i=0; i<6;i++) {
			
			tempWindow.contents.fontFace = $gameSystem.mainFontFace();
			tempWindow.contents.fontSize = $gameSystem.mainFontSize();
			const textState = tempWindow.createTextState(text, 8+7-i, 8+7-i, 0);
			tempWindow.processAllText(textState);
		}
		
		}
		
		tempWindow.drawTextEx(text, 8, 8, 0);
