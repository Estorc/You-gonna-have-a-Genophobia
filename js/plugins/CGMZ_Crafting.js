...
CGMZ_Temp.prototype.pluginCommandCraftingReinitialize = function() {
	for(let i = 0; i < $cgmz._recipes.length; i++) {
		const recipe = new CGMZ_Recipe(JSON.parse(CGMZ.Crafting.Recipes.filter(item => JSON.parse(item).Name === $cgmz._recipes[i]._name)[0]));
		$cgmz._recipes[i]._products = recipe._products
		$cgmz._recipes[i]._failProducts = recipe._failProducts
		$cgmz._recipes[i]._ingredients = recipe._ingredients
		$cgmz._recipes[i]._tools = recipe._tools
	}
};
//-----------------------------------------------------------------------------
