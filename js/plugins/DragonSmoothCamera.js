...
(() => {
	...
	alias_Game_Map_startScroll   = Game_Map.prototype.startScroll,
	alias_Game_Map_updateScroll  = Game_Map.prototype.updateScroll;
	...
	Object.assign(Game_Map.prototype, {
		...
		setDisplayPos() {
			...
			if (typeof this._pixelDisplay === 'undefined') {
				this._pixelDisplay = new PIXI.Point();
				this._restDisplayX = 0;
				this._restDisplayY = 0;
			}
		...
		//------------------------------------------------------------------------
		startScroll() {
			SDragon.SmoothCamera.enabled = false;
			alias_Game_Map_startScroll.call(this, ...arguments);
		},
		//------------------------------------------------------------------------
		// * updateScroll
		// - Aliased function
		//------------------------------------------------------------------------
		updateScroll() {
			alias_Game_Map_updateScroll.call(this, ...arguments);
			if ($gamePlayer.isMoving()) {
				SDragon.SmoothCamera.enabled = true;
			}
		},
		//------------------------------------------------------------------------
		// * scrollDown
		...
		scrollDown(distance) {
			...
			let   pixelDist   = (SDragon.SmoothCamera.enabled) ? Math.floor(tempDist) : tempDist;
	...
	Object.assign(Game_CharacterBase.prototype, {
		...
		screenX() {
			...
			return Math.floor($gameMap.adjustPixelX(this._realX * tw) + tw / 2)+(this[_0x5f1364(0x2db)]||0x0);
		},
		//------------------------------------------------------------------------
		// * screenY
		// - Overwrite function
		...
		screenY() {
			...
			return Math.floor($gameMap.adjustPixelY(this._realY * th) + th - this.shiftY() - this.jumpHeight())+(this[_0x5f1364(0x41a)]||0x0);
		},
	});
	//=============================================================================
	// ** Game_Player
	...
	Object.assign(Game_Player.prototype, {
		...
		cameraFocus() {
			...
			if (typeof this._cameraOffset === 'undefined') {
				this._cameraOffset = {
					x: parseInt(SDragon.SmoothCamera.cameraOffset.x),
					y: parseInt(SDragon.SmoothCamera.cameraOffset.y)
				};
