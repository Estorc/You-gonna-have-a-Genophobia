...
$randomInput = function() {
	...
	let input = ['ok','cancel','up','down','left','right']
	return input[Math.floor(Math.random()*5)];
	
}

$createQTEinRange = function(range) {
	
	let qte = [];
	for (let i = 0; i<range; i++) {
		
		qte.push($randomInput());
...
$createQTEinRangeRythm = function(range,time) {
	
	let qte = [];
	for (let i = 0; i<range; i++) {
		
		qte.push([$randomInput(),time]);
...
Window_QTE.prototype.initialize = function(duration) {
	...
	const rect = new Rectangle(x, y, 1000, 1000);
	...
	console.log(PluginManager.parameters('QTEWindow').GaugeBack);
	this._gaugeBack = "rgba("+a[0]+","+a[1]+","+a[2]+",1)"
	this._gaugeFilling = "rgba("+b[0]+","+b[1]+","+b[2]+",1)"
...
Window_QTE.prototype.update = function() {
	if (!this._frozen) {
		...
		this.drawText(this.getSeqIndex().toString() + '/' + this._sequence.length.toString(), x, y+44, width, "left");
		this.drawText
...
Window_QTE.prototype.drawRythm = function() {
	...
	var height = 36
	var color1 = "rgba(188,64,64,1)"
	var color1Bis = "rgba(255,0,0,1)"
	var color2 = "rgba(64,64,188,1)"
	var color2Bis = "rgba(0,0,255,1)"
	var color, colorBis
	var sum = 0;
	var sum2 = this._sequence[0][1];
...
Game_Map.prototype.QTE = function(mode,duration,sequence,visible,x,y,opacity,width,height,wrongInput,showTime) {
	...
	if (mode[0] == "infernal")  {
		qte.windowskin = ImageManager.loadSystem("InfernalWindow");
		mode[0] = "normal";
	} else {
		qte.windowskin = ImageManager.loadSystem("Window");
	}
	if (mode[0] === "free") {
		sequence = [];
		qte._currInputs = 0;
