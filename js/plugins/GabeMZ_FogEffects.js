...
(() => {
    ...
    Sprite_Fog.prototype._updateMovement = function() {
		...
		if (Math.abs(this.lastDiplayX-$gameMap.displayX()) > 5) {
			this.constX += (this.lastDiplayX-$gameMap.displayX()) * $gameMap.tileWidth();
		}
		this.lastDiplayX = $gameMap.displayX();
		
        this.origin.x = ($gameMap.displayX() * $gameMap.tileWidth()) + this.constX - 96;
        this.constY += this.speedY;
		
		if (Math.abs(this.lastDiplayY-$gameMap.displayY()) > 5) {
			this.constY += (this.lastDiplayY-$gameMap.displayY()) * $gameMap.tileWidth();
		}
		this.lastDiplayY = $gameMap.displayY();
